...
public class T17LockAccountTest {
	/*
	 	락 기능을 제공하는 클래스
	 	- ReentrantLock : Read 및 Write 구분없이사용하기 위한 락 클래스로 동기화 처리를 위해 이용한다.
	 					snychronized를 이용한 동기화 처리보다 부가적인 기능이 제공된다
	 					ex) Fairness 설정 등... => 가장 오래 기다린 스레드가 가자 먼저 락을 획득해야함
	 					
	 	- ReentrantReadWriteLock : Read 및 Write 락을 구분하여 사용가능함
	 				여러 스레드가 동시에 read작업은 가능하지만 write 작업은 단지 하나의 스레드만 가능함.
...
// 입출금 처리 위한 클래스(공유 객체용)
class LockAccount{
	private int balance;
	
	// lock 객체변수 => 되도록이면 private final로 만든다.
	...
	public void deposite(int money) {
		// Lock갤체의 lock()메서드가 동기화 시작이고, unlock()메서드가 동기화의 끝을 나타낸다.
		// lock() 메서드로 동기화를 설정한 곳에는 반드시 unlock()메서드로 해제해 주어야한다.
		lock.lock(); // 락 설정(락을 획득하기 전까지 BLOCKED 상태 됨)
