...
public class T07EnumTest {
	...
	public enum Season {
		...
		//생성자 만들기(열거형의 생성자는 접근제어가 묵시적으로 'private'이다.)
		private Season(String data) {
			this.data = data;
		}
		
		// 값을 꺼내기 위한 getter함수 생성
	...
	public static void main(String[] args) {
		...
		System.out.println("myCity2의 ordinal : " + myCity1.ordinal());
		System.out.println();
		...
		for(City city : City.values()) {
			System.out.println(city + " : " + city.ordinal()); // city == city.toString()
		}
		
		City city = City.대구;
		HomeTown ht = HomeTown.대구;
		System.out.println(city == City.대전); // enum 객체는 '=='로 비교
		System.out.println(city == City.대구);
		//System.out.println(city == ht); // enum끼리의 타입까지 구별시켜줌 그러므로 컴파일에 에러가 생김(비교 의미가 없음
