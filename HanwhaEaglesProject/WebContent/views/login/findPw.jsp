    ...
    <article id="container" class="">
        <div class="conts_black_box">
            ...
            <form action="/" method="post">
                <fieldset>
                    <legend class="d-flex justify-content-center" style="border-bottom: solid gray; font-size: 16px; color: #8a8686; padding-bottom: 10px; margin-bottom: 35px;">한화이글스 회원 비밀번호 찾기</legend>

                        <div style="margin-top: 10px; margin-bottom: 30px; color: white">
                            <label for="userID">
                                <strong class="orangeText">* </strong>아이디　　　　
                            </label>
                    ...
                    <div style="margin-top: 15px; display: flex; justify-content: center;">
                        <input type="button" id="findPwAjax" value="완료" class="col-12 em"
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content bg-black text-white" style="border: 2px solid white">
      <div class="modal-header" style="border-bottom-color: orange;">
        <h5 class="modal-title" id="loginModalLabel">비밀번호 찾기 결과</h5>
      ...
      <div class="modal-footer"  style="border-top-color: orange;">
			<button type="button" class="btn btn-secondary" style="background-color: #f37321;" data-bs-dismiss="modal" onclick="Login()">로그인</button>
			<button type="button" id="closeBtn" class="btn btn-secondary" style="background-color: #f37321;" data-bs-dismiss="modal" onclick="redirectToHomepage()">닫기</button>     
    ...
    <script>
        ...
        $('#findPwAjax').on('click', function(){
       	 ...
       	 $.ajax({
       		    type: 'post',
       		    url: '/FindPw.do',
       		    data: { memId: uId, memName: uName, memMail: uMail},
...
//        		            console.log('result:', data.result == "fail");
       		        ...
       		        if (data.result == "fail") {
       		            modalBody.html('해당 정보와 일치하는 내역이 없습니다.');
       		            $('#loginModal').modal('show');
       		        } else {
       		            // 모달 창에 동적으로 아이디와 가입일자 추가
       		            modalBody.html('<p class="title text-white d-flex justify-content-center" style="font-size: 18px;"> 비밀번호가 메일로 전송되었습니다.</p><p class="d-flex justify-content-center" style="margin-top: 8px; color: #808080;">(가입일 : ' + data.memRegDate + ')</p>');
       		            $('#loginModal').modal('show');
