                ...
                <a href="" class="nav-link text-white">
				    <div class="row collapse navbar-collapse board-div">
                         <ul class="navbar-nav content-div board_menu">	     
                            ...
                            <li class="col col-1" id="PCFACN"><a href="http://localhost:8888/mypage/basketList.do" class="nav-link"><span style="width: 0px; color:orange;">장바구니</span></a></li>                                                     
...
<%
	int listSize = myProductList.size();
	int i = 0;
	if(listSize == 0){
%>
...
<%
		...
		for(MypageProductVO mpv : myProductList) {
			i++;
...
%>	
			...
			<form action="/mypage/basketCancle.do" method="POST">
				<input  type="hidden" name = "vProdCd" value ="<%=mpv.getProdCd() %>">
				<input  type="hidden" name = "vCartCd" value ="<%=mpv.getCartCd() %>">
				<nav class="content-header navbar-expand-lg">
									...
									<div class="row collapse navbar-collapse board-idv">
										<ul class="navbar-nav content-div board_menu" style="padding: 0px 3px 0px 3px;">
                                            ...
                                            <li class="col col-6 board-list" style="color: gray; text-align: center; font-size: 0.8em;">
                                                <a href="http://localhost:8888/shop/product.do?ProdCd=<%=mpv.getProdCd() %>" class="nav-link text-white"><span name="prodName" style="width: 0px;"><%=mpv.getProdName() %></span></a></li>
											<li class="col col-1 board-list" style="color: gray; text-align: center; font-size: 0.8em;">
												<a href="" class="nav-link text-white"><span style="width: 0px;"><%=mpv.getCartDtNum() %></span></a></li>
											<li class="col col-1 board-list" style="color: gray; text-align: center; font-size: 0.8em;">
												<a href="" class="nav-link text-white"><span style="width: 0px;"><%=mpv.getCartDtSize() %></span></a></li>
											<li class="col col-1 board-list" style="color: gray; text-align: center; font-size: 0.8em;">
                                                <a href="" class="nav-link text-white"><span style="width: 0px;"><%=mpv.getProdPrice() %></span></a></li>
                                            <li class="col col-1 board-list" style="color: gray; text-align: center; font-size: 0.8em;">
                                                <a href="" class="nav-link text-white"><span style="width: 0px;"><%=mpv.getNumPrice() %></span></a></li>
                                            <li class="col col-1 board-list" style="color: gray; text-align: center; font-size: 0.8em;">
...
%>
					...
					<div class="row collapse navbar-collapse">
						<ul class="navbar-nav content-div board_menu" style="height: 50px;">
							<li class="col col-9"></li>
								...
								int sum=0;
								for(MypageProductVO mpv : myProductList) {
									sum= sum+(mpv.getNumPrice());
								}
								%>
							<li class="col col-1"><span style="color:white;"><h4>결제합계&nbsp;: </h4></span></li>								
							<li class="col col-1"><span style="color:white;"><h4 name="sum"><%=sum%>&nbsp;원</h4></span></li>
						</ul>
					</div>
				</nav>
				
				<nav class="content-header navbar-expand-lg black-board2">
					<div class="row collapse navbar-collapse">
						<ul class="navbar-nav content-div board_menu" style="height: 50px;">
							<li class="col col-10"></li>							
							<li class="col col-2"><span style="color:white;">&nbsp;&nbsp;&nbsp;
							<form id="payForm" action="/payment/kakao.do" method="POST">						
							<form id="payForm" action="/mypage/basketPay.do" method="POST">
 ...
 <!-- 모달1 -->
 <div class="modal fade" id="payModal" tabindex="-1" aria-labelledby="loginModalLabel" aria-hidden="true" style="z-index=2; ">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content bg-black text-white" style="border: 2px solid white">
      <div class="modal-header" style="border-bottom-color: orange;">
        <h5 class="modal-title" id="loginModalLabel">결제진행</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body" id="byeModalBody" >
      <div style="border:2px solid orange; border-radius:10px; padding : 3px;"> 
      ■ 상품목록<br><hr>    
<%
	int j=0;
	if(listSize == 0){
		...
		for(MypageProductVO mpv : myProductList) {
			j++;
%>			
      	<%=j%>. 상품명 : <%=mpv.getProdName() %>, 수량 : <%=mpv.getCartDtNum() %>개, <br>
      	&nbsp;&nbsp;&nbsp;가격 : <%=mpv.getNumPrice()%>원<br>
      	<hr>
...
%>
		...
		<br>
        	결제를 진행하시겠습니까?. <br> 
        	결제방법 선택창으로 가시려면 <span style="color: orange;"> 확인 </span> 버튼을 클릭해주세요.
      </div>
...
<!-- 모달2 -->
 <div class="modal fade" id="payCheckModal" tabindex="-1" aria-labelledby="loginModalLabel" aria-hidden="true" style="z-index=2;">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content bg-black text-white" style="border: 2px solid white">
      <div class="modal-header" style="border-bottom-color: orange;">
        <h5 class="modal-title" id="loginModalLabel">결제진행 선택</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body" id="payCheckModalBody" >
        	결제방법을 선택해주세요. <br> 
        	선택을 완료하시면 <span style="color: orange;"> 결제창 </span>으로 넘어갑니다.<br>
        	<select id="paySelect" class="form-select" aria-label="Default select example"
        	style="margin : 7px 0px 7px 0px; border : 2px solid orange;">
  			<option selected>결제방법을 선택해주세요</option>
  			<option value="1">카카오페이</option>
			</select>
			결제방법 :&nbsp;<span id="selectContent" style="color:orange;"></span>
      </div>
      <div class="modal-footer" style="border-top-color: orange;">
      <button type="button" class="btn btn-secondary" id="noPay" style="background-color: #f37321;" data-bs-dismiss="modal">뒤로 가기</button>
      <button type="button" class="btn btn-secondary" id="goPay" style="background-color: #f37321;" >확인</button>
</div>
</div>
</div>
</div>
 ...
 <div class="modal fade" id="checkYourPay" tabindex="-1" aria-labelledby="loginModalLabel" aria-hidden="true" style="z-index=2;">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content bg-black text-white" style="border: 2px solid white">
      <div class="modal-header" style="border-bottom-color: orange;">
        <h5 class="modal-title" id="loginModalLabel">결제수단오류</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body" id="byeModalBody" >
        	<span style="color:orange;">결제수단</span>을 선택해주세요. <br> 
      </div>
      <div class="modal-footer" style="border-top-color: orange;">
      <button type="button" class="btn btn-secondary" id="backPaySudan" style="background-color: #f37321;" >확인</button>
</div>
</div>
</div>
</div>
...
$('#paySelect').on('change', function(){
	let paySelect = $("#paySelect option:selected").text();
	$('#selectContent').html("");
	console.log($('#payCheckModalBody'));
	if(paySelect!= "결제방법을 선택해주세요"){
		$('#selectContent').append(paySelect);
	}
});
...
$('#payment').on('click', function(){
	  ...
	  $('#yesBtn').on('click', function(){
		  ...
		  $('#goPay').on('click',function(){	
			  ...
			  if($('#selectContent').text() == "카카오페이"){
			  document.getElementById('payForm').submit();	//카카오로쏴줌  
		  }else {
			  $('#checkYourPay').modal('show'); 
			  $('#payCheckModal').modal('hide'); 
			  $('#backPaySudan').on('click', function(){
				  $('#payCheckModal').modal('show');
				  $('#checkYourPay').modal('hide'); 
			  });
