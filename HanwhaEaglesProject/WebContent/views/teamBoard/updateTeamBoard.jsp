...
<nav class="container-fluid content-board bg-black">
                ...
                <ul class="navbar-nav content-div board_menu" style="margin-top: 80px;">
                    ...
                    <li class="col col-8" id="PCFACN">
                        <h1 class="text-white" style="font-style: bold;">
                            <span style="font-weight: bold;  margin-bottom: 35px; margin-left: 150px;">게시글 수정</span>
                        </h1>
                    </li>
                </ul>
            </div>
 ...
 %>
   ...
   <span id="existingImg" style="display: none;"><%=tbv.getTeamboardImg() %></span>
			...
			<div class="notice_write" >
				<div class="list_form">
					<div class="row_hr" style="height: 50px;">
						...
						<div class="item_row">
							<div class="input_wrap textarea" >
								<textarea class="form-control" rows="1" name="title" id="title"><%=tbv.getTeamboardTitle() %></textarea>
							</div><!-- //.input_wrap -->
						</div>
					</div>
					...
					<label for="title" class="text-white">내용
						...
						<div class="item_row">
							<div class="input_wrap textarea">
								<textarea class="form-control" rows="20" name="content" id="content"><%=tbv.getTeamboardContent() %></textarea>
							</div><!-- //.input_wrap -->
							<div class="list_bul">
<!-- 								<p class="bul_star gray_12">음란, 폭력, 정치적인 내용의 이미지, 영상이나 글이 포함되었을 경우 관리자에 의해 해당 게시글이 비공개 처리될 수 있습니다.</p> -->
				            ...
				            <div class="row_hr">
               <label for="title" class="text-white">이미지 첨부
                  <strong class="label"></strong>
               </label>
               <div class="item_row">
...
<div class="row_hr" style="background-color: black;">
    <label for="is_pinned" class="text-white">고정 여부</label>
    <div class="item_row">
        <div class="input_wrap">
            <select class="form-control" name="is_pinned" id="is_pinned" style="width: 150px;">
...
<div class="paginate btn_area center" id="insertBtn">
			<a href="/TeamBoard.do" type="button" class="btn_com gray"><span style="width: 0px;"></span>취소</a>
			<button type="button" onclick="updateBoard()" class="btn_com em">등록</button>
		</div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content bg-black text-white" style="border: 2px solid white">
      <div class="modal-header" style="border-bottom-color: orange;">
        <h5 class="modal-title" id="loginModalLabel">게시글 수정</h5>
      ...
      <button type="button" class="btn btn-secondary" id="closeBtn" style="background-color: #808080;" data-bs-dismiss="modal">취소</button>
...
$('#yesBtn').on('click', function(){
    ...
    let existingImg = document.getElementById("existingImg").innerText;
    let title = $('#title').val();
    let content = $('#content').val();
    let isPinned = $('#is_pinned').val(); // 고정 여부 값 가져오기
	let imageFile = $('#image')[0].files[0];
    
    var formData = new FormData();
    formData.append('tbCode', teamboardCd);
    formData.append('tbTitle', title);
    formData.append('tbContent', content);
